...
trials_to_bust <- function (bankroll, num_bets, p) {
  # create an empty vector which will contain the amounts of money gambler has after each bet 
  sum <- c(NA, length=num_bets)
  # set the first element of this vector equal to the bankroll
  sum[1] <- bankroll
  
  # simulating a sequence of bets
  for (bet in 2:num_bets) {
    if (sum[bet-1]!=0){
...
iterations <- 5000
# vector containing results from all iterations of trials_to_bust
simulation <- replicate(iterations, trials_to_bust(1000,100,0.5))
...
Pr_get_busted <- length(simulation[simulation<100])/iterations
